...
# The dep_delay attains the highest possible value for the given distance, when the distance value changes to the next smallest value
...
flights %>%
  mutate(total_gain=arr_delay-dep_delay, gain_hour=total_gain/(air_time/60))
```
...
flights %>%
  ...
  group_by(origin) %>%
  summarize(no_flights=n(), mean_arr_del=mean(arr_delay, na.rm=TRUE))
```
